...
public class CrushTurtle extends AdvancedRobot {
    ...
    public void run() {
        ...
        setGunColor(new Color(0, 150, 50)); //FF4A74
        setBulletColor(new Color(255, 255, 100));
        setRadarColor(new Color(40, 78, 180));
        setScanColor(new Color(255, 74, 114, 220));
    ...
    public void onRoundEnded(RoundEndedEvent event) {
        System.gc();
        _radarScanner.onRoundEnded(event);
    }

    public void onBattleEnded(BattleEndedEvent event)
    {
        _radarScanner.onBattleEnded();
    }


    public void doGun() {
        if (_radarScanner._surfAbsBearings.size() == 0 || _radarScanner._lastScan == null)
            return;

        double absBearing = _radarScanner._surfAbsBearings.get(0) - Math.PI;
        ...
        //selectFiringPower(_radarScanner.nme.distance);

		/*
		 * logs a gun wave when we fire;
		 */
        if (getGunHeat() == 0) {
            logFiringWave(_radarScanner.nme, true);
        }
        //	else
        //		logFiringWave(nme, false);
	
		/*
		 * This method checks our waves to see if they have reached the enemy yet.
		 */
        checkFiringWaves(CTUtils.project(new Point2D.Double(getX(), getY()), absBearing, _radarScanner.nme.distance));
        ...
        if (!waveTestMode) {
            ...
            if (_radarScanner.nme.energy == 0) {
                // Head-On Targeting
                double absoluteBearing = getHeading() + _radarScanner.nme.bearing;
                ...
                boolean gunFinishedTurning = 0.0001 >= Math.abs(getGunTurnRemainingRadians());

                //	setTurnGunRightRadians(tempRotate);
                double gunRotate = Utils.normalRelativeAngle(absBearing - getGunHeadingRadians())
                        + gunAngles[8 + (int) (_radarScanner.nme.velocity * Math.sin(_radarScanner.nme.headingRadians - absBearing))];

                if (tempRotate != Double.MAX_VALUE) {
                    gunRotate = tempRotate;  // why not.. this is the KNN learned angle
                    enemyProjectedDistance = (enemyProjectedDistance == 0 ? _radarScanner.nme.distance : enemyProjectedDistance);
                }
...
// gunRotate < Math.atan2(9, enemyProjectedDistance)
                ...
                if (getEnergy() - FIRE_POWER > 0.6 && gunFinishedTurning /* gunRotate < Math.atan2(9, enemyProjectedDistance)/*Math.abs(getGunTurnRemaining()) < 10*/) {
                    if (getTime() > 25)
                        setFire(FIRE_POWER);
                }
                ...
                double deltaHeading = heading - _radarScanner._lastHeading;
                ...
                while (Point2D.distance(x, y, dX, dY) > projectedTime * (20 - 3 * 3) && RadarScanner._fieldRect.contains(dX, dY));

                setTurnGunRightRadians(Utils.normalRelativeAngle(Math.atan2(dX - x, dY - y) - gunHeading));
                setBulletColor(new Color(255, 70, 70));

                if (getEnergy() + 0.6 > FIRE_POWER)
                    setFire(FIRE_POWER);
            }
        }

        _radarScanner._lastHeading = heading;
    }
    ...
    public void onScannedRobot(ScannedRobotEvent e) {
        ...
        Situation s = _radarScanner.processScanEvent(e, !waveTestMode);
...
//(int)((s.WallTriesBack+s.WallTriesForward)) (0..2)
        ...
        //tempRotate = knnGun.projectBearing(s, _radarScanner.nme.location.getX(), _radarScanner.nme.location.getY(), enemyHeading);
        updateWaves();
    ...
    public void onHitByBullet(HitByBulletEvent e) {
        ...
        _radarScanner._oppEnergy += robocode.Rules.getBulletHitBonus(e.getBullet().getPower());
        _radarScanner.nme.enemyShotHits++;

        _hitLocations.add(new Point2D.Double(getX(),getY()));
...
*/
    ...
    public double checkDanger(ArrayList<EnemyWave> waves, int direction) {
        ...
        RobotState[] next = calculateFutureDanger (waves, _radarScanner._myLocation.getX(), _radarScanner._myLocation.getY(),
                absBearing,
                getVelocity(),  8, getHeadingRadians(), 0, getDistanceRemaining(), direction == -1 ? true : false, getTime(), _radarScanner._fieldRect, 800, 600, 140, false );
    ...
    public int findSafestMove(ArrayList<EnemyWave> waves, double attackAngle) {
        ...
        RobotState[] left = calculateFutureDanger (waves, _radarScanner._myLocation.getX(), _radarScanner._myLocation.getY(),
                absBearing,
                getVelocity(),  8, getHeadingRadians(), attackAngle, getDistanceRemaining(), true, getTime(), _radarScanner._fieldRect, 800, 600, 140, false );

        RobotState[] right = calculateFutureDanger (waves, _radarScanner._myLocation.getX(), _radarScanner._myLocation.getY(),
                absBearing,
                getVelocity(),  8, getHeadingRadians(), attackAngle, getDistanceRemaining(), false, getTime(), _radarScanner._fieldRect, 800, 600, 140, false );
        ...
        int graphx = (int)getBattleFieldWidth()-((int)graphWidth+30), graphy=100, cnt=0;
...
*/
            ...
            g.drawString("Mean: " + dangerSum + " Danger: " + df.format(25-next[middleIndex].dangerDensity), (float)getBattleFieldHeight()-20f, 10f);
 ...
 */
        ...
        for (int i = points.length; --i >= 0; ) {
            ...
            double flattener = (getRoundNum()/(double)getNumRounds()) * 5.0 +1;
...
//        int movesToFind = (int)CTUtils.clamp(3 * possibleTicks, possibleTicks, 50); // Presently 3 and 50 are optimal
    ...
    {
        double tr = location.distance(getBattleFieldWidth(), getBattleFieldHeight());
        double tl = location.distance(0, getBattleFieldHeight());
        double br = location.distance(getBattleFieldWidth(), 0);
...
*/
        ...
        if (best.size() == 0) {
            ...
            double arcMovement = e.getVelocity() * Math.sin(e.getHeadingRadians() - _targeting._lastBearing);
        ...
        */
        ...
        if ((_radarScanner.nme.energy-getEnergy()) > 40)
            getAway = 1;
...
//avoid = -0.5 * CTUtils.sign(getDistanceRemaining());
		...
		*/
        ...
        CTUtils.setBackAsFront(this, goAngle);
    }
    ...
    public void logFiringWave(Enemy enemy, boolean isReal) {
        GunWave w = new GunWave();
        w.absBearing = enemy.bearingRadians + getHeadingRadians();
        w.bulletVelocity = FIRE_SPEED;
        w.fireLocation = new Point2D.Double(_radarScanner._lastLocation.getX(), _radarScanner._lastLocation.getY());
...
//                g.drawOval((int) (p2.x - 1), (int) (p2.y - 1), 2, 2);
        ...
        double bestDistance = playerStats.getBestDistance();

        double _shotsTotal = playerStats.getOverallStat().getTotal();
        double _shotAccuracy = playerStats.getOverallStat().getHitRatio() * 100;
